...
const Profile = () => {
  ...
  const [liveReferrals, setLiveReferrals] = useState<number | null>(null);

  React.useEffect(() => {
    fetch("/api/profile")
      .then((res) => (res.ok ? res.json() : null))
      .then((data) => {
        if (data?.user?.numberOfReferrals !== undefined) {
          setLiveReferrals(data.user.numberOfReferrals);
        }
      })
      .catch(() => {});
  }, []);
  ...
  const downloadQR = () => {
    try {
      ...
      const fileSafeName = (saData?.saId || saData?.name || "referral")
        .toString()
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <motion.div {...riseProps} className="text-center mb-12">
          ...
          <p className="text-white/60">
            View and manage your Student Ambassador information
          </p>
        </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            >
              ...
              <p className="text-white font-medium text-lg">
                {saData?.joinDate
                  ? new Date(saData.joinDate).toLocaleDateString("en-US", {
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                    })
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            >
              ...
              <div className="flex gap-2 mb-3">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  onClick={() =>
                    saData?.referralLink && copyToClipboard(saData.referralLink)
                  }
              ...
              <p className="text-white/50 text-xs">
                Share this link to invite other students to join as SA
              </p>
            </motion.div>
            ...
            >
              ...
              <p className="text-white/50 text-xs mt-4 text-center">
                Scan with your phone to share referral
              </p>
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
          ...
          {[
            {
              ...
              value: saData?.eventsRegistered || "0",
              color: "from-[#7a1f2a]",
            },
            {
              label: "Number of Students you Referred",
              value:
                liveReferrals !== null
                  ? liveReferrals
                  : (saData?.numberOfReferrals ?? "0"),
              color: "from-[#2d4f9e]",
            },
            {
              label: "Rank",
              value: saData?.SARank ? `${saData.SARank}` : "N/A",
