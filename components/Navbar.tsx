...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-[92vw]">
      {/* Glass container */}
      <div className="backdrop-blur-3xl bg-white/5 border border-white/15 rounded-4xl shadow-[0_20px_60px_rgba(92,10,10,0.35)] px-6 py-4 transition-all duration-300">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {/* Desktop Navigation */}
          <div className="hidden lg:flex flex-wrap items-center gap-2 xl:flex-nowrap xl:gap-4">
            {navItems.map((item) => (
              <div key={item.path} className="relative group">
                <Link
                  href={item.path}
                  className={`px-5 py-2 rounded-4xl text-base lg:text-lg font-light transition-all duration-300 border block ${
                    pathname === item.path
        ...
        {/* Mobile Navigation */}
        {isMenuOpen && (
          <div className="lg:hidden mt-4 pt-4 border-t border-white/20 shadow-[0_18px_38px_rgba(92,10,10,0.3)] rounded-2xl">
